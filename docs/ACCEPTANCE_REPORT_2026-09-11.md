...
初次严格验收发现 SQLite 与 Evolution Registry 的连接上下文没有保证关闭。已统一改用显式关闭连接的上下文管理器，并将 `ResourceWarning` 与 pytest 不可回收异常纳入默认测试门槛。修复后结果如下：

- MCP 内存 round-trip 单测：1/1 通过。
- 同一 MCP 用例连续 20 次：通过。
- Python 3.13.5 全仓严格测试：86/86 通过，零失败、零跳过、零资源警告。
- Ruff：通过。
- PostgreSQL 17.11 动态端口隔离验收：通过。
- 六个包重建、123 个源码文件和 12 个归档扫描：通过，零敏感信息命中。
- 1000 次写入、100 次检索资源基线：通过，峰值 RSS 73,564,160 bytes。
